...
def count_unique(string1):
    ...
    for word in word_list:
        unique_count[word] = unique_count.get(word, 0) + 1
    ...
    for n in range(len(list1)):
        for i in range(n +1, len(list1)):
            if list1[n] + list1[i] == 0:
                zero_pair = [list1[n], list1[i]]
                zero_pair.sort()
                if zero_pair not in sum_zero:
...
def find_duplicates(words):
    ...
    duplicates_removed = {w for w in words}
...
def pirate_talk(phrase):
    ...
    for word in phrase_list:
        translation = pirate_phrase_dict.get(word,word)
